...
attr_accessor :api_version, :log, :debug, :base_url, :log_file, :puts_exceptions, :reraise_exceptions
	...
	def	login(opts={})
		...
		@base_url = "https://my.rightscale.com/api/acct/#{@account}"
		unless @log.nil?
		...
		@apiobject = RestClient::Resource.new("#{@base_url}",@username,@password)
		rescue => e
		puts "Error: #{e.message}" if @puts_exceptions
		raise if @reraise_exceptions
	end

	def	send(apistring,type = "get", additional_params = {})
    RestClient.log << '-------------------------'
    RestClient.log << Time.now
		@responsecode = ""

		params = { :x_api_version => "#{@api_version}", :api_version => "#{@api_version}" }
		...
		@callstart = Time.now
    		@reply = @apiobject[apistring].send(type.to_sym, params.merge(additional_params))
		@time = Time.now - @callstart

		@apiheader = @reply.headers
    RestClient.log << "headers | #{@apiheader}"
